...
int main(int argc, char* argv[])
{
  ...
  struct SamplerTraits { using AnsatzType = RBMZ2PrSymm<double>; using FloatType = double; };
    ...
    for (const auto & nf : nFiltersArr)
      for (const auto & alpha : alphaArr)
        for (const auto & theta : thetaArr)
        {
          ...
          RBMZ2PrSymm<double> machine(L, nf, nChains);
          const double J = std::sin(theta), h = -std::cos(theta);
          // load parameters
          const std::string prefix = path + "RBMZ2PrSymmLICH-L" + Lstr + "NF" + nfstr + "A" + alphastr + "T" + thetastr + "V" + verstr;
          ...
          LITFIChain<SamplerTraits> sampler(machine, L, h, J, alpha, false, seed, nBlocks, prefix);
