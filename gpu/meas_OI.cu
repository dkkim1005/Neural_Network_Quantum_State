...
int main(int argc, char* argv[])
{
  ...
  const uint32_t nInputs = parser.find<uint32_t>("Ni"),
    nHiddens1 = parser.find<uint32_t>("Nh1"),
    nHiddens2 = parser.find<uint32_t>("Nh2"),
    ...
    ver1 = parser.find<uint32_t>("ver1"),
    ver2 = parser.find<uint32_t>("ver2");
  ...
  struct SamplerTraits { using AnsatzType1 = ComplexFNN<double>;
    using AnsatzType2 = ComplexFNN<double>; using FloatType = double;};
  auto measPtr = make_unique<MeasOverlapIntegral<SamplerTraits>>(m1, m2, nBlocks, seed);
  const auto res1 = measPtr->get_overlapIntegral(ntrials, nWarmup, nMonteCarloSteps);
  std::cout << "# C_12*<\\psi_1|\\psi_2> : " << res1 << std::endl;
  measPtr.reset(new MeasOverlapIntegral<SamplerTraits>(m2, m1, nBlocks, seed));
