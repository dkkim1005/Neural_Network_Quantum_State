...
template <typename float_t>
LinearSolver<float_t>::LinearSolver(const int n):
  kn(n),
  ipiv_(n, 0) {}

template <typename float_t>
void LinearSolver<float_t>::solve(std::complex<float_t> * A, std::complex<float_t> * B, const char uplo)
{
  int info = 1, lwork = -1;
...
template <typename float_t>
void PsuedoInverseSolver<float_t>::solve(std::complex<float_t> * A, std::complex<float_t> * B, const float_t rcond)
{
  int rank, lwork, info;
  // query mode (lwork = -1, finding an optimal workspace)
  lwork = -1;
