...
int main(int argc, char* argv[])
{
  ...
  FFNN<double> m1(nInputs, nHiddens1, nChains), m2(nInputs, nHiddens2, nChains);

  // load parameters: w,a,b
  m1.load(FFNNDataType::W1, path1 + "Dw1.dat");
  m1.load(FFNNDataType::W2, path1 + "Dw2.dat");
  m1.load(FFNNDataType::B1, path1 + "Db1.dat");
  m2.load(FFNNDataType::W1, path2 + "Dw1.dat");
  m2.load(FFNNDataType::W2, path2 + "Dw2.dat");
  m2.load(FFNNDataType::B1, path2 + "Db1.dat");
  ...
  using ansatz_traits = AnsatzeTraits<Ansatz::FFNN, Ansatz::FFNN, double>;
  std::unique_ptr<MeasOverlapIntegral<ansatz_traits> > measPtr(new MeasOverlapIntegral<ansatz_traits>(m1, m2, nBlocks, seed));
